...
export async function connectWallet() {
  if (!(await isConnected())) throw new Error('Freighter not installed. Please install the extension.')
  return getPublicKey()
}
...
async function sendTxWithResult(publicKey, op) {
  ...
  const sigResult = await signTransaction(prepared.toXDR(), { networkPassphrase: NET, network: 'TESTNET' })
  const signedXdr = typeof sigResult === 'string' ? sigResult : sigResult.signedTxXdr
